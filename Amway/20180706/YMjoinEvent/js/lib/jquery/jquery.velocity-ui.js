...
* add define support and fix option.display not pass to animate by Ammon. 
*/   
(function(factory) {
				...
				for (var callIndex = 0; callIndex < effect.calls.length; callIndex++) {
					...
					if (callIndex === 0) {
						...
						if (/In$/.test(effectName)) {
							opts.display = options.display || Container.Velocity.CSS.Values.getDisplayType(element);
						}
					}
