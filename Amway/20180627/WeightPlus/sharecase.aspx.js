...
$$('simplePagination fb', function () {
    ...
    function BindData(rowCount) {
        ...
        for (var i = 0; i < rowCount; i++) {
            ...
            str += '<div class="introduce box-inline"><p>' + oData.FirstParagraph + '</p></div></li>';
        }
    ...
    $(document).on('click', 'div.theme a', function () {
        ...
        var num = $(this).text().substr($(this).text().indexOf('(') + 1, 1);
        console.log(num);
        if (num == "1")
            $('.showcaseslist').show();
        else
    ...
    $('.search-btn.btn-inline').click(function () {
        ...
        location.href = 'showcases.aspx?q=' + encodeURIComponent($('div.case-about-search input[type=text]').val());
    });
    ...
    $(document).on('click', '.fb-share.btn-inline', function () {
        ...
        //}).done(function (AResult) {
        //    setTimeout($.fb.share(Config.ShareURL + "?q=" + encodeURIComponent(uno)),500);
