...
$$('simplePagination fb', function () {
    ...
    function BindData(rowCount) {
        ...
        for (var i = 0; i < rowCount; i++) {
            ...
            str += '<div class="introduce"><p>' + oData.FirstParagraph + '</p></div></li>';
        }
    ...
    $('div.select select').change(function () {
        var num = $('select option:selected').text().substr($('select option:selected').text().indexOf('(') + 1, 1);
    ...
    $('.search-btn.btn').click(function () {
        ...
        location.href = 'showcases.aspx?q=' + encodeURIComponent($('div.searchbar input[type=text]').val());
    });
