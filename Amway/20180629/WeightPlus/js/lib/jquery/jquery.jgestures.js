...
Sample
    ...
    var giMapBoxCurrLeft = parseInt($('#control').css('left'));
    var giMapBoxCurrTop = parseInt($('#control').css('top'));

    $('#control').on('tapone swipemove', function (event_, obj) {
        ...
        switch (_a[0]) {
            case 'tapone':
                if (event.changedTouches && event.changedTouches[0]) {
                    var tapX = event.changedTouches[0].clientX;
                    var tapY = event.changedTouches[0].clientY;

                    console.log('tapone', tapX, tapY);
                }
                break;
            case 'swipemove':
                if (_a[1] != 1) { break; }

                var iNewL = giMapBoxCurrLeft + obj.delta[0].startX;
                var iNewT = giMapBoxCurrTop + obj.delta[0].startY;
                document.getElementById('#control').style.left = iNewL + 'px';
                document.getElementById('#control').style.top = iNewT + 'px';
                giMapBoxCurrLeft = iNewL;
                giMapBoxCurrTop = iNewT;
