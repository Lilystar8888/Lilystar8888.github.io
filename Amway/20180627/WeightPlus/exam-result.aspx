<%@ Page Language="C#" AutoEventWireup="true" CodeFile="exam-result.aspx.cs" Inherits="exam_result" %>
<script runat="server">
    protected override void Render(HtmlTextWriter writer)
    {
        ...
        { 
            case 1:
                META_TITLE="檢測結果-健康樂活型";
                META_DESCRIPTION = @"在飲食上懂得節制，或是工作關係會大量消耗熱量。平時比較注重健康，且有運動習慣。但隨著年齡增長或工作型態改變，飲食、運動及生活型態也須隨之調整。";
                break;
            case 2:
                META_TITLE = "檢測結果-壓力多吃型";
                META_DESCRIPTION = @"生活不規律，吃飯時間非早即晚，往往會以「吃」的方式來紓解壓力，尋求慰藉。壓力一來就無法克制吃的慾望，而且特別喜歡吃高油、高糖、高鹽的高熱量食物，導致體 重節節攀升。此外常常睡眠不足（不到6小時）或是睡眠品質不佳。";
                break;
            case 3:
                META_TITLE = "檢測結果-暴飲暴食型";
    ...
    <div id="wrap">
        <div class="content">
            ...
            <div class="exam-result-content">
                <div class="exam-result01" runat="server" id="result01">
                    <div class="pic"></div>
                    <div class="genius">
                        <div class="title-genius">你的類型屬於…<span>第5類-健康樂活型</span></div>
                ...
                <div class="exam-result02" runat="server" id="result02">
                    <div class="pic"></div>
                    <div class="genius">
                        <div class="title-genius">你的類型屬於…<span>第3類-壓力多吃型</span></div>
                ...
                <div class="exam-result03" runat="server" id="result03">
                    <div class="pic"></div>
                    <div class="genius">
                        <div class="title-genius">你的類型屬於…<span>第4類-暴飲暴食型</span></div>
                ...
                <div class="exam-result04" runat="server" id="result04">
                    <div class="pic"></div>
                    <div class="genius">
                        <div class="title-genius">你的類型屬於…<span>第1類-糖心型</span></div>
                ...
                <div class="exam-result05" runat="server" id="result05">
                    <div class="pic"></div>
                    <div class="genius">
                        <div class="title-genius">你的類型屬於…<span>第2類-代謝偏低型</span></div>
                ...
                <a href="javascript:;" class="btn-exam-download" ><img src="images/icon-download-exam.png" height="28" width="36">下載完整資訊(PDF)</a>
                <a href="javascript:;" class="btn-exam-menu" ><img src="images/icon-menu-exam.png" height="32" width="32">飲食示範菜單(PDF)</a>
                <a href="exam.aspx" class="btn-exam-back"><img src="images/icon-back-exam.png" height="37" width="41">返回體重管理評估</a>
            </div>

            <div class="exam-about">
                <%if (liThisArticleInfo.Count > 0)
                  {%>
                <h4 class="title-right"><%:dicBodyToCategory["0" + resultType] %>相關文章</h4>
                <ul class="writings-list-right">
                    <%for (int i = 0; i < liThisArticleInfo.Count; i++) 
                    { %>
                    <li>
                        <div class="pic"><a href="javascript:;"><img src="API/GetPic.ashx?Token=<%:GetToken() %>&Type=Article&FileName=<%=liThisArticleInfo[i].articleInfo.LargePicName%>"  alt=""/></a></div>
                        <div class="writings-detail"><h3><a href="detail.aspx?no=<%=liThisArticleInfo[i].articleInfo.UNO %>"><%=liThisArticleInfo[i].articleInfo.Title %></a></h3></div>
                        <div class="tag-area">
                            <!--五型-->
                             <%for (int j = 0; j < liThisArticleInfo[i].liBodyTypeInfo.Count; j++)
                               {%>
                            <a href="tag-related.aspx?type=bodytype&p=<%=liThisArticleInfo[i].liBodyTypeInfo[j].UNO %>" class="btn-kind"  data-tracking="ga('send','event','pc','label.type','<%=liThisArticleInfo[i].liBodyTypeInfo[j].Name%>');"><%=liThisArticleInfo[i].liBodyTypeInfo[j].Name%></a>
                            <%} %>
                        </div>
                        <div class="tag-area">
                            <!--營養素-->
                            <%for(int j=0;j<liThisArticleInfo[i].liNutrientInfo.Count;j++) {%>
                            <a href="tag-related.aspx?type=nutrient&p=<%=liThisArticleInfo[i].liNutrientInfo[j].UNO %>" class="btn-nutriment"  data-tracking="ga('send','event','pc','label.nutrient','<%=liThisArticleInfo[i].liNutrientInfo[j].Name%>');"><%=liThisArticleInfo[i].liNutrientInfo[j].Name%></a>
                            <%} %>
                        </div>
                    </li>
                    <%} %>
