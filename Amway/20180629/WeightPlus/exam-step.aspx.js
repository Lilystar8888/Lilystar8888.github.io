...
$$('', function () {
    ga('send', 'pageview', '/exam-step01.aspx');
    $('.rate').width('0%');
    ...
    $('.btn-inline,.ans').bind('touchstart mousedown', function () {
        ...
        $(this).addClass('act');
        if ($('.page' + thispage).find('.act').length == 8 && (thispage != 1 || $('.sex-exam').find('.act').length == 1)) {
    ...
    $('.btn-finish').bind('click', function () {
        if ($(this).hasClass('lightgray')) {
            ...
            $('.act').slice(1).each(function (index) {
                var val = $(this).attr('value');
                var type = $(this).parent().parent().attr('qtype');
                setVal(type, val);
            });
            showResult();
...
function initPage(page) {
    ...
    if (page == 5) {
        $('.next-page').hide();
        $('.btn-finish').css('display', 'inline-block')//.show();
    } else {
        $('.next-page').css('display', 'inline-block')//.show();
    }
    if (page == 1) {
        $('.sex-exam').show();
        $('.prev-page').hide();
    } else {
        $('.prev-page').css('display', 'inline-block')//.show();
    }
    if ($('.page' + page).find('.act').length == 8 && (page != 1 || $('.sex-exam').find('.act').length == 1)) {
        $('.next-page').removeClass('lightgray');
    }
    if ($('.act').length == 41) {
        $('.btn-finish').removeClass('lightgray');
